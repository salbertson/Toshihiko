...
Query.prototype.findOne = function(callback, withJson) {
    ...
    this._limit.limit = 1;
    return this.find(function(err, rows, sql) {
    ...
    if(!this.model.memcached) {
        ...
        this.toshihiko.execute(sql, function(err, rows) {
            if(err) {
                ...
                return promisor._onExecuteResult(err, undefined, sql);
            }
        ...
        this.toshihiko.execute(sql, function(err, rows) {
            if(err) {
                ...
                return promisor._onExecuteResult(err, undefined, self.makeSQL("find"));
            }
            ...
            self.model.memcached.getData(self.toshihiko.database, self.model.name, rows, function(err, data) {
                ...
                var taskDone = function(TO) {
                    ...
                    callback(err, result);
                    promisor._onExecuteResult(err, result);
                };
