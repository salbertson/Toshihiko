...
var Query = function(model) {
    ...
    this.engine = this.toshihiko.engine;
    this.keywordSymbol = this.toshihiko.engine.keywordSymbol;
    this.stringSymbol = this.toshihiko.engine.stringSymbol;
};
...
Query.prototype._makeConditionField = function(condition, field, type) {
    ...
    if(typeof condition !== "object") {
        ...
        var sql = this.keywordSymbol + field.column + this.keywordSymbol + " = ";
        sql += field.type.needQuotes ?
            this.stringSymbol + escaper.escape(condition) + this.stringSymbol :
            condition;
    ...
    var cdtMapper = function(value) {
        ...
        var cdt = (self.keywordSymbol + "{field}" + self.keywordSymbol + " {symbol} {value}").assign({
            field  : field.column,
            symbol : symbol,
            value  : field.type.needQuotes ?
                        self.stringSymbol + escaper.escape(value) + self.stringSymbol :
                        value
        });
    ...
    for(var i = 0; i < keys.length; i++) {
        ...
        if(symbol) {
            ...
            if(value === null) {
                if(symbol === "=") {
                    conditionStrings.push(this.keywordSymbol + field.name + this.keywordSymbol + " IS NULL");
                    continue;
                }
                if(symbol === "!=") {
                    conditionStrings.push(this.keywordSymbol + field.name + this.keywordSymbol + " IS NOT NULL");
                    continue;
                }
            }
...
Query.prototype._makeSet = function() {
    ...
    for(var key in this.updateData) {
        ...
        str += this.keywordSymbol + name + this.keywordSymbol + " = ";
    ...
    return mysql.format(str, arr, undefined, undefined, this.keywordSymbol);
};
...
Query.prototype._makeSQLDelete = function() {
    var sql = "DELETE FROM {k1}{name}{k2} ".assign({
        name: this.model.name,
        k1: this.keywordSymbol,
        k2: this.keywordSymbol
    });
...
Query.prototype._makeSQLUpdate = function() {
    var sql = "UPDATE {k1}{name}{k2} SET ".assign({ 
        name: this.model.name,
        k1: this.keywordSymbol,
        k2: this.keywordSymbol
    });
...
Query.prototype._makeSQLFind = function(type) {
    ...
    var sql = this.fields.reduce(function(sql, field) {
        sql += _idx++ ? ", " : " ";
        return sql + self.keywordSymbol + self.model.nameToColumn(field) + self.keywordSymbol;
    }, "SELECT");
...
Query.prototype.execute = function(sql, format, callback) {
    ...
    if(format) {
        sql = mysql.format(sql, format, undefined, undefined, this.keywordSymbol);
    }
