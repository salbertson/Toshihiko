...
var Yukari = function(model, source) {
    ...
    this.$keywordSymbol = this.$engine.keywordSymbol;
    this.$stringSymbol = this.$engine.stringSymbol;
};
...
Yukari.prototype.insert = function(callback) {
    ...
    var sql = this.$engine.makeInsertSQL(this.$model.name, data);
...
Yukari.prototype.update = function(callback) {
    ...
    // set data
    var sql = "UPDATE {k1}{table}{k2} SET ?";
    var finalData = {};
    for(var key in data) {
    ...
    sql = sql.assign({ table: this.$model.name, k1: this.$keywordSymbol, k2: this.$keywordSymbol });
    sql = mysql.format(sql, finalData, undefined, undefined, this.$keywordSymbol) + " WHERE ";
    ...
    for(var i = 0; i < this.$model.primaryKeys.length; i++) {
        ...
        sql += this.$keywordSymbol + p.column + this.$keywordSymbol + " = ?";

        pk.push(p.type.restore(this.$origData[p.name].data));
    }
    sql = mysql.format(sql, pk, undefined, undefined, this.$keywordSymbol);
